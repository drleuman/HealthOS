...
export function TopNav() {
    ...
    return (
        <>
            {/* P2: Skip link — visible only on keyboard focus */}
            <a
                href="#main-content"
                className="sr-only focus:not-sr-only focus:fixed focus:top-4 focus:left-4 focus:z-[100] focus:rounded-md focus:bg-slate-100 focus:px-4 focus:py-2 focus:text-slate-950 focus:font-semibold focus:shadow-lg"
            >
            ...
            <nav
                ...
                className="sticky top-0 z-50 backdrop-blur-xl bg-slate-950/80 border-b border-slate-800/80 transition-all h-16"
            >
                <div className="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8 h-full">
                    <div className="grid grid-cols-12 h-full items-center gap-4">
                        ...
                        <div className="col-span-9 md:col-span-3 flex items-center justify-end gap-3 sm:gap-4">
                            {/* Login — mantiene /auth ya que el usuario ya tiene cuenta */}
                            <Link
                                href="/auth"
                                onClick={() => trackEvent('landing_cta_click', { button: 'nav_login' })}
                                className="text-xs sm:text-sm font-medium text-slate-400 hover:text-slate-50 transition-colors"
                            >
                                {t('login')}
                            </Link>
                            {/* Start — va directamente al onboarding para usuarios nuevos */}
                            <Link
                                href="/app/onboarding/start"
                                onClick={() => trackEvent('landing_cta_click', { button: 'nav_start' })}
                                className="inline-flex items-center justify-center h-8 sm:h-9 px-4 sm:px-5 rounded-full bg-slate-100 text-slate-950 text-xs sm:text-sm font-bold hover:bg-white hover:scale-105 transition-all shadow-lg shadow-slate-950/20"
                                aria-label={t('start')}
                            >
                                {t('start')}
                            </Link>
